import React, { useState } from "react";
import { AiOutlineClose } from "react-icons/ai";
import { BsThreeDots, BsHeart } from "react-icons/bs";
import { SlLike } from "react-icons/sl";
import { CiChat1 } from "react-icons/ci";
import { RiMessengerLine } from "react-icons/ri";
import CreateComment from "./CreateComment";
const Post = () => {
	const [showMore, setShowMore] = useState(false);
	const fullText = `
	Lorem ipsum, dolor sit amet consectetur adipisicing elit. Consequuntur debitis deleniti mollitia laborum ea ex
	cumque, recusandae magni fuga voluptas earum praesentium nihil similique voluptates velit corporis ducimus
	obcaecati dicta beatae inventore. Non, similique. Similique iusto at harum voluptate dolore consequuntur
	voluptatum aliquam doloribus quam enim quod maiores explicabo assumenda cumque quas, quia nesciunt accusamus
	veniam reiciendis vel consectetur magni mollitia. Id, illum sapiente ea aliquid quasi corrupti! Eum voluptas,
	hic, provident, aliquam sapiente possimus vitae cum perferendis placeat eligendi porro. Sequi impedit et eius.
	Impedit quasi corrupti velit. Inventore neque ratione beatae voluptate aliquam distinctio illum temporibus nobis
	voluptatem.`;

	const truncate = (str, maxLength) => {
		if (str.length > maxLength) {
			return str.slice(0, maxLength) + "...";
		}
		return str;
	};
	const showMoreText = () => setShowMore(!showMore);

	return (
		<main className="bg-white shadow border rounded-md my-1 md:my-3 pb-2">
			<section className="border-b px-2 py-1 md:px-4 md:py-2 flex justify-between items-center ">
				<article className="flex items-center justify-between">
					<aside className="flex gap-x-4">
						<div className="relative">
							<img
								src="https://www.freecodecamp.org/news/content/images/2022/09/jonatan-pie-3l3RwQdHRHg-unsplash.jpg"
								alt="group-profile"
								className="w-10 aspect-square rounded-md "
							/>
							<img
								src="data:image/jpeg;base64,"
								alt="profle"
								className="w-8 aspect-square rounded-full absolute -right-1 -bottom-1"
							/>
						</div>
						<div className="">
							<h5>John Doe</h5>
							<h6>Gully Ktm</h6>
						</div>
					</aside>
				</article>
				<article className="flex text-gray-500">
					<aside className="w-9 aspect-square rounded-full flex justify-center items-center hover:bg-gray-200 cursor-pointer">
						<AiOutlineClose size={24} />
					</aside>
					<aside className="w-9 aspect-square rounded-full flex justify-center items-center hover:bg-gray-200 cursor-pointer">
						<BsThreeDots size={24} />
					</aside>
				</article>
			</section>
			<section className="md:py-2 py-1 border-b ">
				<article className="md:px-5 md:py-3 px-2 py-1">
					<h6>
						{!showMore ? truncate(fullText, 150) : fullText}
						{!showMore ? (
							<button onClick={showMoreText} className="text-blue-400 ml-1">
								see more
							</button>
						) : (
							<button onClick={showMoreText} className="text-blue-400 ml-1">
								see less
							</button>
						)}
					</h6>
				</article>
				<article>
					<img
						src="https://letsenhance.io/static/b8eda2f8914d307d52f725199fb0c5e6/62e7b/MainBefore.jpg"
						alt="post image"
						className="w-full aspect-auto"
					/>
				</article>
			</section>
			<section className="px-4 py-2 border-b">
				<article className="flex items-center justify-between text-gray-500">
					<aside className="flex items-center gap-x-1 ">
						<SlLike />
						<BsHeart />
						<h6>248</h6>
					</aside>
					<aside className="flex items-center gap-x-1">
						<h6>190</h6>
						<h6>Comments</h6>
					</aside>
				</article>
			</section>
			<section className="px-4 py-2 border-b">
				<article className="grid grid-cols-3 place-items-center-center text-gray-500">
					<aside className="flex items-center py-2 cursor-pointer rounded-md justify-center gap-x-2 hover:bg-gray-100 grid-cols-1">
						<SlLike size={18} />
						<h5 className="font-bold text-sm">Like</h5>
					</aside>
					<aside className="flex items-cente">
						<CiChat1 size={18} />
						<h4 className="font-bold text-sm">Comment</h4>
					</aside>
					<aside className="flex items-center py-2 cursor-pointer rounded-md justify-center gap-x-2 hover:bg-gray-100 grid-cols-1">
						<RiMessengerLine size={18} />
						<h4 className="font-bold text-sm">Send</h4>
					</aside>
				</article>
			</section>
			<CreateComment />
			<button className="font-semibold text-gray-500 my-2 ml-5">View more comments</button>
		</main>
	);
};

export default Post;
